...
export function LabTab({ labSelection, setLabSelection, methodLegend = [], labData = {} }) {
  const currentLab = labData[labSelection];
    ...
    <div className="space-y-8 animate-in slide-in-from-left duration-500">
      ...
      <div className="flex overflow-x-auto gap-3 pb-4 no-scrollbar px-1">
        {Object.keys(labData).map(key => (
          ...
          >
            <span className="text-2xl">{labData[key].icon}</span>
            <span className="text-[10px] font-black uppercase tracking-tighter">{labData[key].label}</span>
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-orange-500/10 border border-orange-500/20 rounded-[2rem] flex gap-4 items-center shadow-lg">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-300 italic leading-relaxed">{currentLab.tip}</p>
            </div>
          </div>
        </div>
      )}
